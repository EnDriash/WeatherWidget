...
class Carousel extends React.Component {
    ...
    startDrag = (event) => {
        ...
        if (event.target.className === 'dragRegister') {
            this.dragRegister.current.addEventListener('mousemove', this.contentManager, false)
        } else {
            this.scrollbar.current.addEventListener('mousemove', this.scrollBarManager, false)
        }
    }
    ...
    offsetCarouselInterpolation() {
        ...
        this.bar.current.style.left = `${this.widthInterpolation(this.bar.current.offsetLeft, this.scrollbar.current.offsetWidth)}px`
        this.carouselContentChild.style.left = `${this.widthInterpolation(this.carouselContentChild.offsetLeft, this.carouselContentChild.offsetWidth)}px`
    }


    offsetValidator() {

        const scrollBarEndpoint = this.scrollbar.current.offsetWidth - this.bar.current.offsetWidth
        const carouselContentEndpoint = -this.carouselContentChild.offsetWidth + this.carousel.current.offsetWidth


        if (this.bar.current.offsetLeft < 0 || this.carouselContentChild.offsetLeft > 0) {
            this.bar.current.style.left = `1px`
            this.carouselContentChild.style.left = `-1px`
        }


        if (this.bar.current.offsetLeft > scrollBarEndpoint || this.carouselContentChild.offsetLeft < carouselContentEndpoint) {
            this.bar.current.style.left = `${this.carousel.current.offsetWidth - this.bar.current.offsetWidth}px`
            this.carouselContentChild.style.left = `${-this.carouselContentChild.offsetWidth + this.carousel.current.offsetWidth}px`
        }
    }
    ...
    getDeltaMove(startPoint = false, type, delta) {
        ...
        const scrollRange = this.carousel.current.offsetWidth - this.bar.current.offsetWidth
        const carouselRange = this.carouselContentChild.offsetWidth - this.carousel.current.offsetWidth
    ...
    scrollBarManager = (event) => {
        ...
        this.bar.current.style.left = `${moveX - this.bar.current.offsetWidth / 2}px`
        this.carouselContentChild.style.left = `${(this.state.contentOffsetLeft - this.getDeltaMove(moveX, 'carousel'))}px`
    ...
    contentManager = (event) => {
        ...
        this.setState({ xpoint: moveX })
        this.bar.current.style.left = `${(this.state.barOffsetLeft - this.getDeltaMove(moveX, 'scrollbar'))}px`
    ...
    render() {
        ...
        return (
            <div className="carousel col-11" ref={this.carousel}>
                ...
                <div className="right-coursor" ref={this.rightCoursor} onClick={event => this.moveRightHandler(event)}>
                    <Arrow />
                </div>
                <div className="scrollbar" ref={this.scrollbar} onMouseUp={event => this.endDrag(event)} onMouseDown={event => this.startDrag(event)} >
